...
Delta_u <- exp(-r*l)*(Cuu - Cud)/(Su*(u-d))
Delta_d <- exp(-r*l)*(Cud - Cdd)/(Sd*(u-d))
Cu <- Delta_u * Su + Bu
Cd <- Delta_d * Sd + Bd
B <- exp(-r*h)*((u*Cd-d*Cu)/(u-d))
Delta <- exp(-r*l)*(Cu - Cd)/(s*(u-d))
...
Delta_uu <- exp(-r*l)*(Cuuu - Cuud)/(Suu*(u-d))
Delta_ud <- exp(-r*l)*(Cuud - Cddu)/(Sud*(u-d))
Delta_dd <- exp(-r*l)*(Cddu - Cddd)/(Sdd*(u-d))
